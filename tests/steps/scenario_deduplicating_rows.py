...
@scenario('../features/silver_data.feature', 'De-duplicating rows from source')
...
def data_transformation(test_context):
...
@then(parsers.parse('I should have in silver table only {expected_count:d} distinct rows'))
...
@then('the {duplicated_count:d} rows violating the constraints should be moved into recycle')
